...
func New(log LR.Logger) error {
	...
	err = res.dg.Open()
	if err != nil {
		res.Error("error opening connection: ", err)
		return err
	}
...
func (b *Bot) MessageCreate() {
	handler := func(s *DG.Session, m *DG.MessageCreate) {
		...
		if err != nil {
			...
			//    new ones.
			b.Error("error creating channel:", err)
		...
		if err != nil {
			...
			b.Error("error sending DM message:", err)
			s.ChannelMessageSend(
				m.ChannelID,
